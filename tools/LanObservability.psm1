...
function Get-RouterClientListViaSsh {
        ...
        foreach ($line in $Lines) {
            ...
            foreach ($token in $tokens) {
                ...
                if ($parts.Count -ge 2) {
                    $mac = $parts[0].ToUpper()
                    $ip = $parts[1]
                    $name = ($parts | Select-Object -Skip 2 | Where-Object { $_ -ne '' } | Select-Object -First 1)
                    $map[$mac] = @{
                        ip = $ip
                        name = $name
                    }
    ...
    try {
        ...
        # Static DHCP reservations (manual assignments)
        $staticResult = Invoke-SSHCommand -SessionId $session.SessionId -Command "nvram get dhcp_staticlist" -TimeOut $TimeoutSeconds -ErrorAction SilentlyContinue
        if ($staticResult.Output) {
            $staticLeases = Parse-StaticLeases -Lines $staticResult.Output
        }
        ...
        # Get wireless clients per band
        $bandCommands = @(
            @{ Band = '2.4ghz'; InterfaceName = 'wl0' },
            @{ Band = '5ghz'; InterfaceName = 'wl1' }
        )

        foreach ($bandCmd in $bandCommands) {
            $band = $bandCmd.Band
            $ifaceName = $bandCmd.InterfaceName
            $wlResult = Invoke-SSHCommand -SessionId $session.SessionId -Command "wl -i $ifaceName assoclist" -TimeOut $TimeoutSeconds -ErrorAction SilentlyContinue
            if ($wlResult.Output) {
                foreach ($line in $wlResult.Output) {
                    if ($line -match '([0-9a-fA-F:]{17})') {
                        ...
                        $rssiResult = Invoke-SSHCommand -SessionId $session.SessionId -Command "wl -i $ifaceName rssi $mac" -TimeOut 5 -ErrorAction SilentlyContinue
                        $rssi = $null
                        ...
                        $leaseType = 'DHCP'
                        $hostname = $null
                        if ($staticLeases.ContainsKey($mac)) {
                            $leaseType = 'Manual'
                            if (-not $ip) { $ip = $staticLeases[$mac].ip }
                            $hostname = $staticLeases[$mac].name
                        }
