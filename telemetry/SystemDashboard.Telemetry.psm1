...
function Read-SystemDashboardConfig {
    ...
    if ($raw.PSObject.Properties.Name -contains 'telemetry') {
        ...
        if ($t.events) { $raw.Service.Events = $t.events }
    }
    ...
    if (Test-Path -LiteralPath $LogPath) {
        try {
            ...
            if ($currentSize -ge $maxBytes) {
                ...
                $archive = Join-Path $dir "$name.$stamp$ext"
                try {
                    Move-Item -LiteralPath $LogPath -Destination $archive -Force -ErrorAction Stop
                }
                catch {
                    $archive = Join-Path $dir "$name.$stamp.$([Guid]::NewGuid().ToString('N'))$ext"
                    Move-Item -LiteralPath $LogPath -Destination $archive -Force -ErrorAction Stop
                }

                $archives = @(Get-ChildItem -LiteralPath $dir -Filter "$name.*$ext" -File -ErrorAction SilentlyContinue |
            ...
            # Best-effort rotation only; do not break telemetry on log I/O issues.
        }
    }
...
function Start-TelemetryService {
    ...
    $asusStateDir = $null
    if ($config.Service.Asus.StatePath) {
        $asusStateDir = Split-Path -Parent $config.Service.Asus.StatePath
    }
    ...
    foreach ($dir in @($config.Service.Syslog.BufferDirectory, $config.Service.Asus.DownloadPath, $asusStateDir, $eventsStateDir, $ingestion.StagingDirectory)) {
    ...
    try {
        ...
        Write-TelemetryLog -LogPath $logPath -Message "Failed to bind syslog UDP listener to $($config.Service.Syslog.BindAddress):$($config.Service.Syslog.Port) - $($_.Exception.Message). Another process may already be using this port." -Level 'ERROR'
    ...
    try {
        while (-not $cts.IsCancellationRequested) {
            ...
            if ($now -ge $nextFlush) {
                Write-TelemetryLog -LogPath $logPath -Message "Flush window reached. syslog_buffer=$($syslogBuffer.Count) asus_buffer=$($asusBuffer.Count)" -Level 'INFO'
