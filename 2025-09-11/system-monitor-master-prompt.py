# Create a JSON prompt spec file for the System Monitoring Web Application
import json, datetime, textwrap, os, pathlib
...
prompt_spec = {
    ...
    "description": (
        "Generate a production-ready home PC monitoring web application that collects Windows Event Logs, "
        "network telemetry, router client lists, and syslogs, then uses the OpenAI API to assess errors/trace logs "
        "and suggest configuration fixes. Outputs should be shippable, secure-by-default, and easy to operate."
    ),
    "persona": {
    ...
    "capabilities": {
        "event_logs": {
            ...
            "rest": ["GET /api/events?log=System&level=Error&since=PT24H&skip=0&take=200"]
        },
        "network_telemetry": {
            "metrics": ["CPU", "memory", "disk", "NIC throughput"],
            "commands": ["Get-Counter", "Get-NetTCPConnection (for TCP/UDP counts)"],
            "rest": ["GET /api/metrics", "STREAM /stream/metrics (SSE or WebSocket)"]
        },
        "router_clients": {
            ...
            "config": {"Router": {"Provider": "AsuswrtSsh", "Host": "192.168.1.1", "Port": 22, "User": "admin"}},
            "rest": ["GET /api/router/clients"]
        },
        "syslog_ingestion": {
            "options": [
            ...
            "normalize": ["timestamp", "host", "facility", "severity", "message", "tags"],
            "rest": ["GET /api/syslog?host=&severity=&since=PT1H"]
        },
        "openai_integration": {
            "purpose": "Advisory engine to analyze errors/trace logs and propose configuration suggestions",
            "endpoints": ["POST /api/ai/assess"],
            ...
            "analysis_persona": "Seasoned Ops diagnostician — concise, actionable, explains why"
        },
        "web_ui": {
            "pages": ["Overview", "Event Logs", "Network", "Router Clients", "Syslog", "Ask AI"],
            "features": [
        ...
        "operations": {
            ...
            "health": ["GET /healthz (dependencies)", "GET /readyz (db + collectors ready)"],
        ...
        "security_posture": {
            ...
            "headers": ["X-Content-Type-Options", "Referrer-Policy", "Content-Security-Policy (documented)"]
        },
        "docs": {
            "deliverables": [
    ...
    "output": {
        ...
        "feature_flags": ["postgres_optional", "syslog_udp_optional"]
    },
    "concrete_example_prompt": (
        "Write a production-ready System Monitoring web application using PowerShell 7+ with Pode (backend) and "
        "HTMX + vanilla HTML/CSS (frontend). It must monitor Windows Event Logs (Application, System); network telemetry "
        "(CPU, memory, disk, NIC throughput; TCP connection stats); router client list (provider model: default ASUSWRT over SSH; optional SNMP fallback); "
        "syslogs (poll SolarWinds files OR optional UDP 514 listener); and provide OpenAI advisory to analyze batched errors/trace logs and suggest configuration remediations "
        "with redaction and rate-limit guards.\n\n"
        ...
        "Key features:\n"
        "- Backend routes: GET /api/events, GET /api/metrics, GET /api/router/clients, GET /api/syslog, POST /api/ai/assess, GET /healthz, GET /readyz; "
        "plus one streaming endpoint: /stream/metrics (SSE).\n"
...
out_path = "G:\\Storage\\BenStuff\\Development\\SystemDashboard\\2025-09-11\\system-monitor-master-prompt.json"
pathlib.Path(out_path).write_text(json.dumps(prompt_spec, indent=2, ensure_ascii=False), encoding="utf-8")
out_path
