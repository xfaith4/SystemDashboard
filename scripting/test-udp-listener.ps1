...
try {
    ...
    Write-Host "   Send test messages using: .\scripting\test-syslog-sender.ps1" -ForegroundColor Gray
    Write-Host ""
    ...
    if ($messageCount -eq 0) {
        Write-Host "💡 No messages received. Try running: .\scripting\test-syslog-sender.ps1 -Port $Port" -ForegroundColor Yellow
    }
}
catch {
