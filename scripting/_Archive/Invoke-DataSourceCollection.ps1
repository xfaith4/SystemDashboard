...
# Resolve staging dir relative to repo root
$repoRoot = Split-Path -Parent $scriptRoot
if (-not [System.IO.Path]::IsPathRooted($StagingDirectory)) {
    $StagingDirectory = Join-Path -Path $repoRoot -ChildPath $StagingDirectory
}

# Load DataSourceManager module
$dsModulePath = Join-Path -Path $repoRoot -ChildPath 'tools\DataSourceManager.psm1'
