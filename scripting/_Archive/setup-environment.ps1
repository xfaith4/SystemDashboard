...
.EXAMPLE
.\scripting\setup-environment.ps1
Set variables for current session only

.EXAMPLE
.\scripting\setup-environment.ps1 -Permanent
...
$EnvVars = @{
    'ROUTER_LOG_PATH' = Join-Path $RepoRoot 'sample-router.log'
    'SYSTEMDASHBOARD_ROOT' = $RepoRoot
}
...
if ($Permanent) {
    ...
    Write-Host "   .\scripting\setup-environment.ps1 -Permanent" -ForegroundColor Blue
}

Write-Host "`n🧪 Running validation to verify setup..." -ForegroundColor Cyan
python (Join-Path $ScriptDirectory 'validate-environment.py')
